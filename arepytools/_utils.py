...
_ERROR_CHECK_FIRST_AXIS_SIZE_OF_NUMPY_ARRAY = (
    "Vector {} has wrong first axis size: {} != {}"
)
...
def check_size_of_numpy_array(
    vector: np.ndarray, size, name="", throw_on_error=True
) -> bool:
    """
    ...
    if size != vector.size:
        if throw_on_error:
            raise RuntimeError(
                _ERROR_CHECK_SIZE_OF_NUMPY_ARRAY.format(name, vector.size, size)
            )
        return False
    return True


def check_dtype_of_numpy_array(
    vector: np.ndarray, dtype, name="", throw_on_error=True
) -> bool:
    """
    ...
    if dtype != vector.dtype:
        if throw_on_error:
            raise RuntimeError(
                _ERROR_CHECK_DTYPE_OF_NUMPY_ARRAY.format(name, vector.dtype, dtype)
            )
        return False
    return True


def check_shape_of_numpy_array(
    vector: np.ndarray, shape, name="", throw_on_error=True
) -> bool:
    """
    Checks the data type (dtype) of vector, raises a RunTimeError if not as expected.
    ...
    if shape != vector.shape or len(shape) != len(vector.shape):
        if throw_on_error:
            raise RuntimeError(
                _ERROR_CHECK_SHAPE_OF_NUMPY_ARRAY.format(name, vector.shape, shape)
            )
        return False
    return True


def check_ndim_of_numpy_array(
    vector: np.ndarray, ndim, name="", throw_on_error=True
) -> bool:
    """
    ...
    if ndim != vector.ndim:
        if throw_on_error:
            raise RuntimeError(
                _ERROR_CHECK_NDIM_OF_NUMPY_ARRAY.format(name, vector.ndim, ndim)
            )
        return False
    return True


def check_first_axis_size_of_numpy_array(
    vector: np.ndarray, first_axis_size, name="", throw_on_error=True
) -> bool:
    """
    ...
    if first_axis_size != vector.shape[0]:
        if throw_on_error:
            raise RuntimeError(
                _ERROR_CHECK_FIRST_AXIS_SIZE_OF_NUMPY_ARRAY.format(
                    name, vector.shape[0], first_axis_size
                )
            )
        return False
    return True
...
def input_data_to_numpy_array_with_checks(
    data, dtype=None, size=None, shape=None, ndim=None, first_axis_size=None, name=""
) -> np.ndarray:
    """
    ...
    check_numpy_array(
        ...
        shape=shape,
        ndim=ndim,
        first_axis_size=first_axis_size,
        name=name,
    )
    return data_array
...
def check_numpy_array(
    ...
    if dtype is not None:
        correctness = correctness and check_dtype_of_numpy_array(
            data, dtype, name=name, throw_on_error=throw_on_error
        )
    if size is not None:
        correctness = correctness and check_size_of_numpy_array(
            data, size, name=name, throw_on_error=throw_on_error
        )
    if shape is not None:
        correctness = correctness and check_shape_of_numpy_array(
            data, shape, name=name, throw_on_error=throw_on_error
        )
    if ndim is not None:
        correctness = correctness and check_ndim_of_numpy_array(
            data, ndim, name=name, throw_on_error=throw_on_error
        )
    if first_axis_size is not None:
        correctness = correctness and check_first_axis_size_of_numpy_array(
            data, first_axis_size, name=name, throw_on_error=throw_on_error
        )
    return correctness
...
def check_exists(path_to_file, tag=None):
    ...
    if not os.path.exists(path_to_file):
        raise RuntimeError(
            "{}: {} not found".format(
                "Requested file" if tag is None else tag, path_to_file
            )
        )
        ...
        def decorated_fun(*args_fun):
            for position in positions:
                if position < len(args_fun):
                    check_exists(
                        args_fun[position], "Argument in position {}".format(position)
                    )
            return fun(*args_fun)
